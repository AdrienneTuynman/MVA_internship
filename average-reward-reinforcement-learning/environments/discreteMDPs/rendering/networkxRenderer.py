...
class GraphRenderer:
    ...
    def initRender(self, env):
        ...
        G = nx.MultiDiGraph(action=0, rw=0.0)
        for s in env.states:
            for a in env.actions:
                for ssl in env.P[s][a]:  # ssl = (p(s),s, 'done')
                    G.add_edge(
                        s, ssl[1], action=a, weight=ssl[0], rw=env.R[s][a].mean()
                    )
        ...
        nx.draw_networkx_nodes(
            G, pos, node_size=400, node_color=["tab:gray" for s in G.nodes()]
        )
        nx.draw_networkx_labels(G, pos, font_size=12, font_family="sans-serif")

        for n in G:
            c = Circle(pos[n], radius=0.2, alpha=0.0)
            ax.add_patch(c)
            G.nodes[n]["patch"] = c
        ...
        for u, v, d in G.edges(data=True):
            ...
            if alpha > 0:
                ...
                if u != v:
                    ...
                    n1c = [
                        n1.center[0] + 0.1 * (2 * counts[u][v] + scalearrow),
                    ...
                    if d["rw"] > 0:
                        countsR[u][v] = countsR[u][v] + 1
                        pos[u] = [
                            pos[u][0] + 0.1 * (2 * countsR[u][v] + scalearrow),
    ...
    def render(self, env, current, lastaction, lastreward):
        ...
        nx.draw_networkx_nodes(
            ...
            node_size=400,
            node_color=[
                "tab:gray" if s != current else "tab:orange" for s in self.G.nodes()
            ],
        )
        plt.savefig(
            self.screenshotpath
